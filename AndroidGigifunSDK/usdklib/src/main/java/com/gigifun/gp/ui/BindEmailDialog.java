...
//    private Boolean isEye = false;
  ...
  //  private Dialog loadingDialog;
    ...
    public void initUI(){
       ...
       // mDialog.setContentView(MResource.getIdByName(mActivity, "layout", "dialog_bindemail"));
...
//                mDialog.setContentView(MResource.getIdByName(mActivity, "layout", "dialog_bindemail_ver"));
//                LogUtil.k("竖屏----");
//                break;
//            default:
//                mDialog.setContentView(MResource.getIdByName(mActivity, "layout", "dialog_bindemail"));
...
//        }
    ...
    private void parsebindJosn(String josn) {
    ...
    public void onClick(View v) {
         if (v.getId()==MResource.getIdByName(mActivity, "id", "edt_psw")){
            ...
            UhttpUtil.post(UgameUtil.getInstance().BINDEMAILURL,map, new UcallBack() {

                @Override
                public void onResponse(String response, int arg1) {
                    LogUtil.k("url="+ UgameUtil.getInstance().BINDURL+map.toString());
                    //Toast.makeText(mActivity,"修改成功",Toast.LENGTH_SHORT).show();
                    closeProgressWheel();
                    mDialog.dismiss();
                    //解析response,根据返回吗，吐司相应错误
                    parsebindJosn(response);
                }
