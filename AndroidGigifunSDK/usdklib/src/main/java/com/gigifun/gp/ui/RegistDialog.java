...
public class RegistDialog implements View.OnClickListener{
   ...
   // private Button registFastBtn;
    ...
    private EditText passWordConBtn;
...
//                mDialog.setContentView(MResource.getIdByName(mActivity, "layout", "dialog_register"));
//                LogUtil.k("横屏----1");
//                break;
//            //竖屏
//            case Configuration.ORIENTATION_PORTRAIT:
//                mDialog.setContentView(MResource.getIdByName(mActivity, "layout", "dialog_register_ver"));
//                LogUtil.k("竖屏----");
//                break;
//            default:
//                mDialog.setContentView(MResource.getIdByName(mActivity, "layout", "dialog_register"));
...
//        }
      ...
      //  loadingDialog=LoadingDialog.createLoadingDialog(mActivity);
        ...
        //快速注册
       // registFastBtn= (Button) mDialog.findViewById(MResource.getIdByName(mActivity, "id", "btn_registfast"));
        registBtn = (Button) mDialog.findViewById(MResource.getIdByName(mActivity, "id", "btn_regist"));
        backBtn = (ImageView) mDialog.findViewById(MResource.getIdByName(mActivity, "id", "btn_back"));
        userNameEt = (EditText) mDialog.findViewById(MResource.getIdByName(mActivity, "id", "edt_reg_username"));
        passWordEt = (EditText) mDialog.findViewById(MResource.getIdByName(mActivity, "id", "edt_reg_psw"));
        passWordConBtn = (EditText) mDialog.findViewById(MResource.getIdByName(mActivity, "id", "edt_reg_ensurepsw"));

        layouRes = (LinearLayout)mDialog.findViewById(MResource.getIdByName(mActivity, "id", "layout_res"));

        backBtn.setOnClickListener(this);
        //registFastBtn.setOnClickListener(this);
    ...
    private void parseRegistJosn(String josn) {
        ...
        try {
            ...
            if ("1".equals(status) && "100".equals(code)) {// 可以登录
                ...
                edit.putString("registname", userNameTxt);
                edit.putString("registpassword", passWrodTxt);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case 111:
                    ...
                    break;
                case 400:
                    Toast.makeText(mActivity,MResource.getIdByName(mActivity, "string", "successful_registration"), Toast.LENGTH_SHORT).show();

                    String Access_token = (String) msg.obj;

                    //注册追踪
                    LanucherMonitor.getInstance().registrationTrack(mActivity,sdkUid,"Ugame");
                    mDialog.dismiss();
                    onRegistListener.onRegistSuccessful(userNameTxt,passWrodTxt,sdkUid);
    ...
    public void onClick(View v) {
        if (v.getId()==MResource.getIdByName(mActivity, "id", "btn_regist")){
            userNameTxt = userNameEt.getText().toString().trim();
            passWrodTxt = passWordEt.getText().toString().trim();
            emailTxt = passWordConBtn.getText().toString().trim();
            ...
            UhttpUtil.post(UgameUtil.getInstance().REGURL,map, new UcallBack() {

                @Override
                public void onResponse(String response, int arg1) {
                    LogUtil.k("url="+ UgameUtil.getInstance().REGURL+map.toString());
                    LogUtil.k("passWrodTxt="+passWrodTxt);
                    parseRegistJosn(response);
                }
